...
def process_chunk_with_summary(chunk_content: str, bucket_name: str, output_path: str, chunk_index: int) -> Dict[str, Any]:
    """
    Process a chunk by generating a detailed summary with all important details intact
    """
    print(f"[CHUNK] Starting detailed summary generation for chunk {chunk_index}")
    
    try:
        # Create detailed summary prompt
        summary_prompt = f"""You are an expert technical analyst. Create a comprehensive summary of this mainframe documentation chunk that preserves ALL important technical details for AWS modernization.
...
1. **BUSINESS FUNCTIONS & PROCESSES**
   ...
   - Data processing patterns and rules
   - Integration points and dependencies

2. **TECHNICAL COMPONENTS**
   - Programs, modules, and their relationships
   - Database schemas and data structures
   - File formats and I/O operations
   - Error handling and validation logic

3. **MODERNIZATION OPPORTUNITIES**
   - AWS services that map to each component
...
CRITICAL: Preserve all technical details, code snippets, data structures, and business logic. This summary will be used for cross-chunk analysis and AWS artifact generation."""

        # Generate summary using LLM
        summary = call_llm_converse(summary_prompt, timeout_seconds=120)
        
        if summary.startswith("Error:"):
            return {'status': 'error', 'error': summary}
        
        # Save summary to S3
        summary_key = f"{output_path}/chunk-summaries/chunk_{chunk_index}_summary.md"
        ...
        print(f"[SUMMARY] Generated {len(summary)} character detailed summary for chunk {chunk_index}")
        ...
        return {
            ...
            'processing_type': 'detailed_summary'
        }
...
def lambda_handler(event, context):
    """
    Lambda handler for processing chunks with detailed summaries
    """
    try:
        ...
        print(f"[CONFIG] Detailed summary generation enabled")
        ...
        # Update job status
        update_job_status(job_id, 'PROCESSING', f"Processing chunk {chunk_index} of {total_chunks} with detailed summary generation")
        ...
        # Process chunk with detailed summary
        print(f"[PROCESSING] Using detailed summary generation for chunk {chunk_index}")
        
        summary_result = process_chunk_with_summary(chunk_content, bucket_name, output_path, chunk_index)
        
        if summary_result['status'] == 'error':
            error_message = f"Error in summary generation for chunk {chunk_index}: {summary_result['error']}"
        ...
        return {
            ...
            'summary_key': summary_result['summary_key'],
